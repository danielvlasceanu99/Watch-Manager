...
export class UserWatchlistComponent implements OnInit {
    ...
    getMovies() {
        if (this.user?.movieWatchlist !== undefined) {
            this.isLoading = true;
            this.movieService.getbyList(this.user?.movieWatchlist).subscribe((res) => {
                this.movies = res;
    ...
    getTvShows() {
        if (this.user?.tvWatchlist !== undefined) {
            this.isLoading = true;
            this.tvService.getbyList(this.user?.tvWatchlist).subscribe((res) => {
                this.tvShows = res;
