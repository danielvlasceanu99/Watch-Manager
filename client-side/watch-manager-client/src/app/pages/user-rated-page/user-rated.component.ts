...
@Component({
    ...
    styleUrls: ["./user-rated.component.scss"],
    animations: [
...
export class UserRatedComponent implements OnInit {
    ...
    getMovies() {
        if (this.user?.ratedMovies !== undefined) {
            const ids = this.user.ratedMovies.map((movie) => movie.id);
            this.isLoading = true;
            this.movieService.getbyList(ids).subscribe((res) => {
                this.movies = res;
    ...
    getTvShows() {
        if (this.user?.ratedTv !== undefined) {
            const ids = this.user.ratedTv.map((movie) => movie.id);
            this.isLoading = true;
            this.tvService.getbyList(ids).subscribe((res) => {
                this.tvShows = res;
